...
どちらの動詞も、ブリッジがコンパイルできないTypeScriptをレンダリングする場合は実行を拒否します（終了コード`2`）— 識別子の重複（同名の宣言が2つある場合や、命名同型によるフィールドの衝突。`col_1`と`col1`はどちらも`col1`になります）、または束縛できない名前（`kebab-case`、`function`のような予約語、空文字列）です。`Bridge::try_render`は同じチェックをプログラムから利用でき、`Bridge::defects`はレンダリングせずに一覧を返します。

メンバーの**キー**は意図的な例外です：ワイヤキーはserdeがJSONに書き出すものそのものなので、`#[serde(rename = "kebab-case")]`は拒否されるのではなく`"kebab-case": T`としてレンダリングされます。
